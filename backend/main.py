...
class KissaCore:
    ...
    def search_candidates(self, query):
        """Recherche Optimisée (Lecture directe des données brutes pour la vitesse)"""
        import logging
        logger = logging.getLogger(__name__)
        
        logger.info(f"⚡ Recherche rapide pour : {query}")
        
        try:
            # 1. Requête API unique
            ...
            for item in results:
                ...
                # ACCÈS DIRECT AUX DONNÉES BRUTES (Évite les requêtes supplémentaires)
                # item.data contient le JSON brut renvoyé par la recherche.
                # C'est instantané car tout est déjà en mémoire.
                d = item.data
                
                # Filtrage Type (Master ou Release uniquement)
                # Le type est souvent dans 'type', parfois implicite
                res_type = d.get('type')
                if res_type not in ['master', 'release']:
                    continue
                
                try:
                    # Titre complet (Souvent "Artist - Album")
                    full_title = d.get('title', 'Inconnu')
                    
                    # Découpage intelligent du titre pour séparer Artiste et Album
                    # Discogs renvoie souvent "Apparat - The Devil's Walk" dans le titre
                    if " - " in full_title:
                        parts = full_title.split(" - ", 1)
                        artist = parts[0]
                        title = parts[1]
                    else:
                        artist = "Artiste Divers / Inconnu"
                        title = full_title
                    
                    # Année
                    year = d.get('year', '')
                    
                    # Label (Souvent une liste dans 'label', on prend le premier)
                    label_raw = d.get('label', [])
                    label = label_raw[0] if isinstance(label_raw, list) and label_raw else str(label_raw)
                    
                    # Image (Thumb est fourni directement dans la recherche)
                    thumb = d.get('thumb', '') or d.get('cover_image', '')
                    
                    candidates.append({
                        "discogs_id": item.id,  # L'ID reste accessible directement
                        "title": title,
                        "artist": artist,
                        "year": str(year),
                        "label": label,
                    ...
                    logger.warning(f"⚠️ Skip item rapide: {inner_e}")
                    continue
            
            logger.info(f"✅ {len(candidates)} candidats trouvés (Mode Rapide)")
            return candidates
